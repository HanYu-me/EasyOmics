    ...
    sidebarPanel(
      ...
      conditionalPanel(condition="input.Function == 'Phenotype Analysis'",
                       ...
                       selectInput("analysis_type", 
                                   label = "Analysis type",
                                   choices = list("Single_Trait","Multil_Traits"),
...
server=function(input, output) {
    ...
    observeEvent(input$runAnalysis,{
      ...
      if(input$Function=="Data Matching"){
        ...
        system(paste0(plink," --vcf ",input$vcf_match$datapath," --keep inter_result/","keepid.txt"," --maf 0.03 --indep-pairwise 20 10 0.99 --allow-extra-chr --recode vcf --out ",paste0(out,"Matched_before_vcf")))
      ...
      if(input$Function=="Phenotype Analysis"){
        ...
        }else if(input$analysis_type=="Multil_Traits"&file.exists((paste0(out,"Rawtra_phenotype_summary.png")))){
          output$phe_plot1 <- renderImage({
            list(src = paste0(out,"Rawtra_phenotype_summary.png"), contentType = "image/png",height = 800)
          }, deleteFile = FALSE)  
        }else if(input$analysis_type=="Single_Traits"&file.exists((paste0(out,"Single_Trait_result.png")))){
            output$phe_plot1 <- renderImage({
