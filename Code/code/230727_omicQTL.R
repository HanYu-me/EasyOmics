...
omicQTL_fun=function(omic,type,gtf,vcf,norm,out,threshold){
  ...
  snps=c()
  nums=c()
  all_qtl=na.omit(all_qtl)
  for(i in unique(all_qtl$gene)){
      finddata=find_qtl(all_qtl[all_qtl$gene==i,])
      snps=c(snps,finddata)
      nums=c(nums,length(finddata))
  }
  snp_freq=data.frame(table(snps))
  names(snp_freq)=c("QTL","freq")
  Gene_freq=data.frame("Prob"=unique(all_qtl$gene),"freq"=nums)
  write.table(snp_freq,file=paste0(out,"qtls_freq.txt"),quote = F,row.names = F,col.names = T)
  write.table(Gene_freq,file=paste0(out,"probs_freq.txt"),quote = F,row.names = F,col.names = T)
  ## remove linked QTL
  ...
  if(sum(diff(chr_info[,2])>0)!=(nrow(chr_info)-1)){
    for(i in chr_info[,1]){
      if(i!=1){
        ...
        chr_mean_pos=c(chr_mean_pos,sum(chr_info[1:(i-1),2])+chr_info[i,2]/2)
      }else{
        chr_mean_pos=c(chr_info[1,2]/2)
      }      
    }
  } 
  snp_pos=snp_pos[,2]
  ...
  axis(1,chr_mean_pos,unique(vcf_data[,1]),las=1,cex.axis=1)
  axis(2,chr_mean_pos,unique(vcf_data[,1]),las=1,cex.axis=1)
  dev.off()
