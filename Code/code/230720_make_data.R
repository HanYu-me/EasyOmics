...
data=cbind(all_exp[,c(1,2)],all_exp[,"AT1G66100"])
names(data)[3]="AT1G66100"

sub=all_exp[,c("family","id","AT4G02260")]
write.table(data,file="../input_V2/GWAs/exp_AT1G66100.txt",row.names=F,col.names=T,quote=F)


a=snp_freq[order(snp_freq$freq,decreasing=T)[c(1:23,25)],1]
summary(as.numeric(str_split_fixed(a,":",2)[,2]))
...
gene
data=data.frame(fread("Analysis_Result\\TWAs\\moa.moa"))
plot(density(-log10(data$p)))
quantile(data$p,0.05)
table(data$p<0.0630025)
